...
public class NamedThreadPoolManager {
    ...
    private CopyOnWriteArrayList<NamedThreadPoolExecutor> namedThreadPoolExecutors = new CopyOnWriteArrayList<>();
    private AtomicBoolean taskStarted = new AtomicBoolean(false);
    private ExecutorService innerThreadPool;

    @Subscribe
    public void namedThreadPoolCreationEventListener(NamedThreadPoolExecutor arg) {
         ...
         */
        NamedThreadPoolExecutor executor = arg;
        namedThreadPoolExecutors.add(executor);
        startCollectTask();
    }
    ...
    public void startCollectTask() {
        ...
        if (taskStarted.compareAndSet(false, true)) {
            innerThreadPool = NamedExecutors.newThreadPoolExecutor(SdkConstants.INNER_POOL, 2, 3, 5, TimeUnit.MINUTES, 1);
    ...
    public CopyOnWriteArrayList<NamedThreadPoolExecutor> getNamedThreadPoolExecutors() {
        return namedThreadPoolExecutors;
    }
}
